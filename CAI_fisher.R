library(XLConnect)
wb<-loadWorkbook("C:/Users/weidewind/Documents/Asymmetry/2015/DataSetsProcessed/BasicDataStale.xlsx")
...
sink("Shon", append=TRUE, split=TRUE)
myRows <- integer()
for(name in sheetNames[10]){
  print(name)
  myDf <- read.xls ("C:/Users/weidewind/Documents/Asymmetry/2015/DataSetsProcessed/BasicData.xlsx", sheet = name, header = TRUE, blank.lines.skip=T, stringsAsFactor=F)
  df<-data.frame(Strand=myDf$"Strand", Essential=myDf$"Essential_population", CAI=as.numeric(myDf$"CAI"))
